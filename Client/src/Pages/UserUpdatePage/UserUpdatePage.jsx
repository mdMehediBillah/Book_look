...
const UserUpdatePage = () => {
  ...
  const navigate = useNavigate();
  const { user, setUser } = useAuthContext();
  const inputRef = useRef(); // Added ref to handle the prompt input
  // const userLocal = localStorage.getItem("user");
  // const user = JSON.parse(userLocal);
  const {
    firstName,
  ...
  const handelUpdate = async (e) => {
    ...
    try {
      let imageUrl = formData.image;
      if (!imageUrl) {
        imageUrl = await uploadImageToCloudinary(imagePreview);
      }
      const updatedFormData = {
        ...formData,
        image: imageUrl,
  ...
  return (
    <main className="pb-16">
      ...
      <div className="py-2 px-4 container mx-auto justify-between screen-max-lg bg-gray-100 max-w-screen-lg pb-16">
        ...
        <label
         htmlFor="aboutMe"
         className="pl-2 text-gray-500 font-semibold"
        >
         About Me
        </label>
        <textarea
         ...
         value={formData.aboutMe}
         onChange={handleChange}
         className="p-2 rounded bg-gray-200 border-none outline-none focus:outline-cyan-300 h-32"
        />
       </div>{" "} */}
              <div className="flex flex-col gap-2 mb-8">
                <label
                ...
                <input
                  ...
                  onChange={handleChange}
                  // ref={inputRef}
                  className="h-10 p-2 rounded bg-gray-200 border-none outline-none focus:outline-cyan-300 w-full"
                />
              </div>
              <div className="flex gap-2 mb-8">
                <button
                  className="btn btn-primary btn-sm max-w-xs"
              ...
              {shareButton && (
                <button
                  className="btn btn-success btn-sm max-w-xs mt-2"
              ...
              {error && (
                <div className="text-red-500 font-semibold mb-8">{error}</div>
              )}
              {success && (
